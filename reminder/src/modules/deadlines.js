...
async function handleInteraction(interaction) {
    ...
    if (interaction.isButton() && interaction.customId === 'btn_mark_done') {
        ...
        try {
            ...
            if (records.length === 0) {
                return await interaction.editReply({ content: '🎉 You have no pending tasks to mark as done!' });
            }
            ...
            await interaction.editReply({ content: '⚠️ Failed to fetch your tasks from Airtable.' });
        }
    }
    ...
    if (interaction.isStringSelectMenu() && interaction.customId === 'select_mark_done') {
        ...
        try {
            ...
            await interaction.editReply({ content: `✅ Successfully marked **${taskName}** as Done!` });
            ...
            await interaction.editReply({ content: '❌ Failed to update the task status in Airtable.' });
        }
    }
